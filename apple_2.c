...
int enqueue(char *client_buf, int size)
{
   ...
   memcpy(client_buf, buf.buf + buf.write_index, part1);  //memcpy won't take care overlap cases, memmove can be used
...
static char *GetErrorString(int x)
{
   ...
   char *str1 = "Success -- No error.";
   char *str2 = "Overflow!";
   
   switch ( x )
   {
       case 0:
           //errorString = "Success -- No error.";            //!! Can't intialize C array like this
           strcpy(errorString, str1);
           break;
       case 2:
           //errorString = "Overflow!";
           strcpy(errorString, str2);
           break;
   }

   //errorString[19] = 0;
   return errorString;              //returning pointer to local varaible!!
}
...
{
   // initialize buffer
   buf.buf = malloc(BUF_SIZE);    //!!!usually malloc call returns void * pntr
   buf.size = BUF_SIZE;
